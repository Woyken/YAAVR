...
public class RagdollHelper : MonoBehaviour
{
    ...
    public delegate void StateChangeEventHandler(RagdollState state);
    public event StateChangeEventHandler onStateChanged;

    private void SetState(RagdollState state)
    {
		this.state = state;
        ...
        set
        {
            if (value == true)
            {
                if (state == RagdollState.animated || state == RagdollState.blendToAnim)
                {
                    ...
                    SetState(RagdollState.ragdolled);
                }
            }
            else
            {
                if (state == RagdollState.ragdolled)
                {
                    ...
                    SetState(RagdollState.blendToAnim);
    ...
    void LateUpdate()
    {
        ...
        if (state == RagdollState.blendToAnim)
        {
            if (Time.time <= ragdollingEndTime + mecanimToGetUpTransitionTime)
            {
                ...
                Vector3 newRootPosition = characterRoot.position + animatedToRagdolled;
                ...
                characterRoot.position = newRootPosition;
                ...
                characterRoot.rotation *= Quaternion.FromToRotation(animatedDirection.normalized, ragdolledDirection.normalized);
            }
